...
pub async fn notify_turn_result<C: TgClient + Sync>(
    ...
    if output.success {
        ...
        let msg = format!("⏰ {} 執行失敗：{}", job.name, failure_summary(&output.message));
...
mod tests {
    ...
    fn turn_job(scope: &str) -> Job {
        Job {
            id: "job-1".to_string(),
            name: "晨間報告".to_string(),
            kind: JobKind::Turn { scope: scope.to_string(), prompt: "go".to_string() },
            cron: "0 9 * * *".to_string(),
            enabled: true,
            next_run_at: Some(1),
            last_run_at: None,
    ...
    async fn delivers_brief_plain_text_on_failure() {
        let client = MockTgClient::default();
        let out = ExecutionOutput { success: false, message: "boom\nstack line 2\nline 3".to_string() };
        let sent = notify_turn_result(&client, &turn_job("user:tg-7"), &out).await.unwrap();
        assert!(sent);
        let log = client.sent.lock().unwrap();
        assert_eq!(log.len(), 1);
        ...
        assert!(!log[0].text.contains("line 3"), "only the first line is included");
    }

    #[tokio::test]
    async fn sends_placeholder_when_success_body_is_empty() {
        let client = MockTgClient::default();
        let sent = notify_turn_result(&client, &turn_job("user:tg-9"), &ok("   ")).await.unwrap();
        assert!(sent);
        let log = client.sent.lock().unwrap();
        assert_eq!(log.len(), 1);
    ...
    async fn skips_non_telegram_scope() {
        let client = MockTgClient::default();
        let sent = notify_turn_result(&client, &turn_job("project:Wukong"), &ok("hi")).await.unwrap();
        assert!(!sent);
    ...
    async fn skips_maintenance_job() {
        ...
        let job = Job {
            ...
            kind: JobKind::Maintenance { scope: Some("user:tg-1".to_string()), task: MaintenanceTask::Consolidate },
            cron: "0 2 * * *".to_string(),
            enabled: true,
            next_run_at: Some(1),
            last_run_at: None,
        };
        let sent = notify_turn_result(&client, &job, &ok("done")).await.unwrap();
        assert!(!sent);
        assert!(client.sent.lock().unwrap().is_empty());
    }
}
