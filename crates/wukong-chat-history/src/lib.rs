...
impl ChatHistoryStore {
    ...
    pub async fn latest_live_event_id(&self, scope: &str) -> Result<Option<i64>, sqlx::Error> {
        let row: Option<sqlx::sqlite::SqliteRow> =
            sqlx::query("SELECT MAX(id) as max_id FROM chat_live_events WHERE scope = ?1")
                .bind(scope)
                .fetch_optional(&self.pool)
                .await?;
...
mod tests {
    ...
    async fn latest_live_event_id_returns_max_id_for_scope() {
        let store = store().await;
        assert_eq!(
            store.latest_live_event_id("user:tg-12").await.unwrap(),
            None
        );
        ...
        assert_eq!(
            store.latest_live_event_id("user:tg-99").await.unwrap(),
            None
        );
    }
