...
- **Adaptive gate**：過短／全停用詞的瑣碎查詢直接略過召回；CJK 查詢使用字元權重判斷，像 `連線` 這類短中文關鍵詞仍會召回，`可以`、`謝謝` 等低資訊量輸入會跳過。
- **信心值與 telemetry**：回應 `confidence` 使用 top hit 的 decay-free `relevance = max(lexical, semantic)`，避免舊但高度相關的記憶被時間衰減壓低信心。每次 recall 會 best-effort 記錄 telemetry（只存 query hash、模式、top_k、scope、命中數、top relevance、skip flag、latency）。
...
`/v1/remember` 與 `/v1/recall` 對 malformed JSON、未知 `kind`、空 items、空白 query 回傳 `400`，語意驗證由核心 `wukong-memory` 共用。
