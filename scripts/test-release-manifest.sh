...
printf '%s\n' "$compatibility" > "$tmp/data-compatibility.json"
scripts/generate-release-manifest.sh --tag v0.18.0-rc.1 --commit 0123456789012345678901234567890123456789 --channel rc --image-reference ghcr.io/raybird/wukong:v0.18.0-rc.1 --image-digest sha256:0123456789012345678901234567890123456789012345678901234567890123 --platform linux/amd64 --runtime-inputs "$runtime_inputs" --data-compatibility "$tmp/data-compatibility.json" --output "$tmp/release-manifest.json"
python3 -c 'import json,sys; data=json.load(open(sys.argv[1])); assert data["productTag"] == "v0.18.0-rc.1"; assert data["promotedFrom"] is None; assert data["runtimeInputs"]["BASE_IMAGE"].startswith("debian:"); assert data["dataCompatibility"]["rollbackSafeTo"] == "v0.17.1"' "$tmp/release-manifest.json"
scripts/generate-release-manifest.sh --tag v0.18.0 --commit 0123456789012345678901234567890123456789 --channel stable --image-reference ghcr.io/raybird/wukong:v0.18.0 --image-digest sha256:0123456789012345678901234567890123456789012345678901234567890123 --platform linux/amd64 --runtime-inputs "$runtime_inputs" --data-compatibility "$tmp/data-compatibility.json" --output "$tmp/stable-manifest.json"
python3 -c 'import json,sys; data=json.load(open(sys.argv[1])); assert data["channel"] == "stable"; assert data["promotedFrom"] is None; assert data["image"]["buildOriginTag"] == "v0.18.0"' "$tmp/stable-manifest.json"
